...
int main(int argc, char **argv)
{
    ...
    std::cout << "Num of clouds: " << dataset_size << std::endl;

    // tqdm loop
    for (const fs::path &entry : tq::tqdm(path_vector))
    {
        pcl::PointCloud<PointIN>::Ptr input_cloud_xyzln = arvc::readPointCloud<PointIN>(entry);
        pcl::PointCloud<pcl::PointXYZL>::Ptr input_cloud (new pcl::PointCloud<pcl::PointXYZL>);
        pcl::copyPointCloud(*input_cloud_xyzln, *input_cloud);
